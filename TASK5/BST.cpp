#include "BST.h"
bool node::exists(node * root, string a)
{
	if (root == nullptr)
	{
		return false;
...
int node::search(node * root, string a)
{
	if (root == nullptr)
	{
		return -1;
...
node * node::insert(node * root, string a, int b)
{
	if (exists(root, a))
	{
		return root;
...
node * node::deleteNode(node * root, string key)
{
	if (root == NULL)
	{
		return root;
